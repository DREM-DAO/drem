<template>
  <MainLayout>
    ...
    <div
      ...
      "
      class="panel panel-primary my-3"
    >
      <h1>Test</h1>
      <button class="btn btn-primary" @click="appCall">App call</button>
    </div>
              ...
              <tr v-for="asset in account['assets']" :key="asset['asset-id']">
                <td>
                  <button
                    ...
                    @click="assetFilter = asset['asset-id']"
                  >
                    {{
...
export default {
  ...
  watch: {
    ...
    async assetFilter() {
      const searchData = await this.searchForTransactionsWithAddrAndAsset({
        addr: this.$route.params.account,
        asset: this.assetFilter,
      });
      if (searchData) {
  ...
  methods: {
    ...mapActions({
      ...
      searchForTransactionsWithAddrAndAsset:
        "indexer/searchForTransactionsWithAddrAndAsset",
      setTransaction: "wallet/setTransaction",
      getAsset: "indexer/getAsset",
      prolong: "wallet/prolong",
      algodexBuy: "algod/algodexBuy",
    ...
    async appCall() {
      ...
      /*
      await this.makeApplicationNoOpTxn({
        creator: this.account,
        appIndex: 22045503,
      });
      */
